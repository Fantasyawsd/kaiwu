...
这里的“算法文档初稿”应当参考当前正式算法文档结构，包含除训练信息以外的全部内容，并为后续人工补充训练/测试截图预留目录。
...
- 训练全部曲线截图和测试截图都统一放入 `screenshots/`
- 图片文件名（去掉扩展名）默认视作上传到官网的模型全称
...
└── screenshots/       # 训练完成后人工放入训练/测试截图；
                       # 图片文件名（去掉扩展名）默认视作官网模型全称
```
...
4. 训练完成后人工操作：
   ...
   - 对开放 10 图做 5 次官方评估，并将测试截图 → 放入 screenshots/
   - 图片文件名（去掉扩展名）视作上传到官网的模型全称
   - AI 后续只检测 screenshots/ 中是否有图，不校验截图真伪
