...
- 补齐训练启动时的 ckpt 续训逻辑：
- 若 `conf/configure_app.toml` 中配置出的 `preload_model_dir + preload_model_id` 能解析到真实 ckpt 文件，则自动从该 ckpt 恢复模型。
- 若目标 ckpt 不存在，则自动回退为从头训练。
- 若从 ckpt 恢复，则 episode / curriculum / eval 节奏也要尽量沿用 ckpt 中保存的进度，而不是重新从 0 开始。

## 当前实现进度

- 已定位到当前续训链路落点：
- `conf/configure_app.toml` 提供 ckpt 目录与 ckpt id。
