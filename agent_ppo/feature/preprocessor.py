...
class Preprocessor:
    ...
    def feature_process(self, env_obs, last_action):
        ...
        dist_shaping = Config.DIST_SHAPING_COEF * (cur_min_dist_norm - self.last_min_monster_dist_norm)
    ...
    def _explore_bonus(self, hero_pos):
        """Reward entering less-visited coarse cells.
        ...
        novelty_ratio = 1.0 / np.sqrt(float(visit_count))
    ...
    def _grid_cell(self, hero_pos):
        ...
        x_idx = min(grid_size - 1, max(0, int(hero_pos["x"] / bucket_size)))
        z_idx = min(grid_size - 1, max(0, int(hero_pos["z"] / bucket_size)))
        return x_idx, z_idx
